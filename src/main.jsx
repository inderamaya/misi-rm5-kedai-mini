      ...
      <section className="missionCard" style={{position: 'relative'}}>
        ...
        <div className="moneyGrid">
          {moneyChoices.map(m => (
            <button
              key={m.id}
              className={`platform-card ${chosen === m.id ? 'selected' : ''} ${feedback?.id === m.id ? (feedback.type === 'correct' ? 'feedback-correct' : 'feedback-wrong') : ''} ${guideMode && m.id !== 'rm5' ? 'guided-inactive' : ''} ${guideMode && m.id === 'rm5' && !chosen ? 'guide-highlight' : ''}`}
              onClick={() => {
                setChosen(m.id);
                if (m.id === 'rm5') {
          ...
          {flowSteps.map((s, idx) => {
            ...
            return (
              <div key={idx} style={{display: 'flex', alignItems: 'center', gap: '10px'}}>
                <button
                  className={`stepping-stone ${isActive ? 'active' : ''} ${feedback?.index === idx ? (feedback.type === 'correct' ? 'feedback-correct' : 'feedback-wrong') : ''} ${guideMode && !isActive && !isNext ? 'guided-inactive' : ''} ${guideMode && isNext ? 'guide-highlight' : ''}`}
                  onClick={() => {
                    if (idx === done.length) {
